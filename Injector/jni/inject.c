...
int main(int argc, char** argv) {
    pid_t target_pid;
    target_pid = find_pid_of("system_server");//also can pass target pid or target process by param
    if (-1 == target_pid) {
        printf("Can't find the process\n");
        return -1;
    }
    inject_remote_process(target_pid, "/data/libpackage.so", "main_entry",  "parameter", strlen("parameter"));
    return 0;
}
